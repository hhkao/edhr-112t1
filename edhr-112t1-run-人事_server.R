...
SELECT [name] FROM [plat5_edhr].[dbo].[row_tables]
	where sheet_id = (SELECT [id] FROM [plat5_edhr].[dbo].[row_sheets]
      ...
      #####111公+私合併#####
      drev_person_pre <-
        bind_rows(drev_person_pre_1111, drev_person_pre_1112)
      
      #檢查本次是否有新資料，若否，則不往下執行
      data_teacher_check <- data_teacher %>%
        mutate(count = 1)
      data_teacher_check <-
        aggregate(count ~ organization_id, data_teacher_check, sum)
      ...
      if (exists("data_teacher_check_pre")) {
        data_teacher_check <-
          left_join(data_teacher_check, data_teacher_check_pre, by = "organization_id") %>%
      ...
      data_staff_check <- data_staff %>%
        mutate(count = 1)
      data_staff_check <-
        aggregate(count ~ organization_id, data_staff_check, sum)
      ...
      if (exists("data_staff_check_pre")) {
        data_staff_check <-
          left_join(data_staff_check, data_staff_check_pre, by = "organization_id") %>%
          rename(count = count.x, count_pre = count.y)
      } else{
      ...
      data_retire_check <- data_retire %>%
        mutate(count = 1)
      data_retire_check <-
        aggregate(count ~ organization_id, data_retire_check, sum)
      ...
      if (exists("data_retire_check_pre")) {
        data_retire_check <-
          left_join(data_retire_check, data_retire_check_pre, by = "organization_id") %>%
          rename(count = count.x, count_pre = count.y)
      } else{
      ...
      data_teacher_check$count_pre[is.na(data_teacher_check$count_pre)] <-
        0
      data_staff_check$count_pre[is.na(data_staff_check$count_pre)] <- 0
      data_retire_check$count_pre[is.na(data_retire_check$count_pre)] <-
        0
      ...
      if (all(data_teacher_check$count == data_teacher_check$count_pre) &
        ...
        data_schtype$authority[substr(data_schtype$organization_id, 3, 3) == "3" |
                                 substr(data_schtype$organization_id, 3, 3) == "4"] <- "縣市立"
        ...
        # type_code
        data_schtype$type_code <- recode_factor(
  ...
  where (substring(a.id, 4, 1) = '1' OR substring(a.id, 4, 1) = '2' OR substring(a.id, 4, 1) = '3' OR substring(a.id, 4, 1) = '4' OR substring(a.id, 4, 1) = 'B' OR substring(a.id, 4, 1) = 'C' OR substring(a.id, 4, 1) = 'F' OR substring(a.id, 4, 1) = 'G') and len(a.id) = 6"
        ...
        temp <- c("0", "1", "2", "3")
        for (x in temp) {
          flag_person$err_flag <- case_when((grepl("科主任", flag_person[[paste("admintitle", x, sep = "")]]) |
                                               (
                                                 grepl("主任", flag_person[[paste("admintitle", x, sep = "")]]) &
        ...
        flag_person$err_flag <- case_when((
          flag_person$type_code == "M" & flag_person$flag_nclass >= 4
        ) ~ 1,
        TRUE ~ 0)
        #---
        ...
        flag_person$err_flag_txt <- case_when(
          flag_person$err_flag_admintitle1 == 1 ~ paste(
            flag_person$name,
            "（",
        ...
        flag_person$err_flag_txt <- case_when(
          flag_person$err_flag_admintitle2 == 1 ~ paste(
            flag_person$name,
            "（",
        ...
        flag_person$err_flag_txt <- case_when(
          flag_person$err_flag_admintitle3 == 1 ~ paste(
            flag_person$name,
            "（",
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          flag16 <- flag16 %>%
            ...
            mutate(flag16 = paste(
              flag16,
              "（請確認或修正請假類別，或是否屬於請假，若以上人員未有請假情事，請填寫半型大寫『N』）",
              sep = ""
            ))
        } else{
        ...
        flag_person$err_flag <-
          if_else(
            grepl("^[^\\s][ABCD89][^\\s]+$", flag_person$idnumber) &
              (flag_person$nation %in% c("本國籍", "本國", "臺灣")),
            1,
            flag_person$err_flag
          )
        ...
        flag_person$err_flag_bdegreeu1 <-
          if_else(
            grepl("^陸軍官校專科班$", flag_person$bdegreeu1),
            0,
            flag_person$err_flag_bdegreeu1
          )
        flag_person$err_flag_bdegreeu1 <-
          if_else(
            grepl("^陸軍官校大學部$", flag_person$bdegreeu1),
            0,
            flag_person$err_flag_bdegreeu1
          )
        ...
        flag_person$err_flag_bdegreeu2 <-
          if_else(
            grepl("^陸軍官校專科班$", flag_person$bdegreeu2),
            0,
            flag_person$err_flag_bdegreeu2
          )
        flag_person$err_flag_bdegreeu2 <-
          if_else(
            grepl("^陸軍官校大學部$", flag_person$bdegreeu2),
            0,
            flag_person$err_flag_bdegreeu2
          )
        ...
        flag_person$tser_ndesem <-
          ceiling((
            flag_person$tser_ndese - floor(flag_person$tser_ndese)
          ) * 12)
        ...
        flag_person$err_flag <-
          if_else((
            flag_person$err_admintitle1 == flag_person$err_admintitle2
          ) & flag_person$err_admintitle1 != "NN",
          1,
          flag_person$err_flag
          )
        flag_person$err_flag <-
          if_else((
            flag_person$err_admintitle1 == flag_person$err_admintitle3
          ) & flag_person$err_admintitle1 != "NN",
          1,
          flag_person$err_flag
          )
        flag_person$err_flag <-
          if_else((
            flag_person$err_admintitle2 == flag_person$err_admintitle3
          ) & flag_person$err_admintitle2 != "NN",
          1,
          flag_person$err_flag
          )
        ...
        flag_person$err_flag_txt <- ""
        flag_person$err_flag_txt <- case_when(flag_person$err_adm == 1 ~ flag_person$name,
                                              TRUE ~ flag_person$err_flag_txt)
        ...
        #呈現姓名
        flag_person$err_flag_txt <- ""
        flag_person$err_flag_txt <- case_when(flag_person$err_lev == 1 ~ flag_person$name,
                                              TRUE ~ flag_person$err_flag_txt)
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^均質化承辦人$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^均職化承辦人$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^優質化協辦人$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("編制外行政人力$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^充實行政人力$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^實習餐廳經理$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^書院Coach$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_admintitle0 <-
          if_else(
            grepl("^專任行政人力$", flag_person$admintitle0),
            0,
            flag_person$err_admintitle0
          )
        flag_person$err_admintitle0 <-
        ...
        flag_person$err_adminunit0 <-
          if_else(
            grepl("^教官室\\(軍訓室\\)$", flag_person$adminunit0),
            0,
            flag_person$err_adminunit0
          )
        flag_person$err_adminunit0 <-
        ...
        flag_person$err_adminunit0 <-
          if_else(
            grepl("^圖資室兼技術交流處$", flag_person$adminunit0),
            0,
            flag_person$err_adminunit0
          )
        flag_person$err_adminunit0 <-
        ...
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^均質化承辦人$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^均職化承辦人$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^優質化協辦人$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
        ...
        flag_person$err_admintitle1 <-
          if_else(
            grepl("編制外行政人力$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
        ...
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^實習餐廳經理$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
        ...
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^電競專案教練$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
        ...
        flag_person$err_admintitle1 <-
          if_else(
            grepl("^臨時約聘助理(計時)$", flag_person$admintitle1),
            0,
            flag_person$err_admintitle1
          )
        flag_person$err_admintitle1 <-
        ...
        flag_person$err_adminunit1 <-
          if_else(
            grepl("^圖資室兼技術交流處$", flag_person$adminunit1),
            0,
            flag_person$err_adminunit1
          )
        flag_person$err_adminunit1 <-
        ...
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^均質化承辦人$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^均職化承辦人$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^優質化協辦人$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
        ...
        flag_person$err_admintitle2 <-
          if_else(
            grepl("編制外行政人力$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
        ...
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^實習餐廳經理$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
        ...
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^電競專案教練$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
        ...
        flag_person$err_admintitle2 <-
          if_else(
            grepl("^臨時約聘助理(計時)$", flag_person$admintitle2),
            0,
            flag_person$err_admintitle2
          )
        flag_person$err_admintitle2 <-
        ...
        flag_person$err_adminunit2 <-
          if_else(
            grepl("^圖資室兼技術交流處$", flag_person$adminunit2),
            0,
            flag_person$err_adminunit2
          )
        flag_person$err_adminunit2 <-
        ...
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^均質化承辦人$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^均職化承辦人$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^優質化協辦人$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
        ...
        flag_person$err_admintitle3 <-
          if_else(
            grepl("編制外行政人力$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
        ...
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^實習餐廳經理$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
        ...
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^電競專案教練$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
        ...
        flag_person$err_admintitle3 <-
          if_else(
            grepl("^臨時約聘助理(計時)$", flag_person$admintitle3),
            0,
            flag_person$err_admintitle3
          )
        flag_person$err_admintitle3 <-
        ...
        flag_person$err_adminunit3 <-
          if_else(
            grepl("^圖資室兼技術交流處$", flag_person$adminunit3),
            0,
            flag_person$err_adminunit3
          )
        flag_person$err_adminunit3 <-
        ...
        #呈現姓名
        flag_person$err_flag_txt <- ""
        flag_person$err_flag_txt <- case_when(flag_person$err_spe == 1 ~ flag_person$name,
                                              TRUE ~ flag_person$err_flag_txt)
        ...
        flag_person$err_flag_adegreeu2 <-
          if_else(
            grepl("Darmstadt$", flag_person$adegreeu2),
            1,
            flag_person$err_flag_adegreeu2
          )
        #副學士的情況
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          spe5 <- spe5 %>%
            ...
            mutate(
              spe5 = paste(
                spe5,
                "（請確認以上人員畢業證書所載學位別。若副學士或專科畢業學校為(科技/空中)大學、(技術)學院或其他技職校院，且確認為專科學制，請於「副學士或專科畢業學校」欄位中在校名後註記專科學制或專科部）",
                sep = ""
              )
            )
        } else{
        ...
        flag_person$err_ddegreeu1 <-
          if_else(
            grepl("Conservatory", flag_person$ddegreeu1),
            0,
            flag_person$err_ddegreeu1
          )
        flag_person$err_ddegreeu1 <-
        ...
        flag_person$err_ddegreeu1 <-
          if_else(
            grepl("TheNewSchool", flag_person$ddegreeu1),
            0,
            flag_person$err_ddegreeu1
          )
        flag_person$err_ddegreeu1 <-
        ...
        flag_person$err_ddegreeu1 <-
          if_else(
            grepl("StellenboschUni$", flag_person$ddegreeu1),
            0,
            flag_person$err_ddegreeu1
          )
        flag_person$err_ddegreeu1 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$ddegreeu1),
            0,
            flag_person$err_ddegreeu1
          )
        flag_person$err_ddegreeu1 <-
        ...
        flag_person$err_ddegreeu2 <-
          if_else(
            grepl("Conservatory", flag_person$ddegreeu2),
            0,
            flag_person$err_ddegreeu2
          )
        flag_person$err_ddegreeu2 <-
        ...
        flag_person$err_ddegreeu2 <-
          if_else(
            grepl("TheNewSchool", flag_person$ddegreeu2),
            0,
            flag_person$err_ddegreeu2
          )
        flag_person$err_ddegreeu2 <-
        ...
        flag_person$err_ddegreeu2 <-
          if_else(
            grepl("StellenboschUni$", flag_person$ddegreeu2),
            0,
            flag_person$err_ddegreeu2
          )
        flag_person$err_ddegreeu2 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$ddegreeu2),
            0,
            flag_person$err_ddegreeu2
          )
        flag_person$err_ddegreeu2 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("Universidad$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("Conservatory$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("TheNewSchool$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("Universitat$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("StellenboschUni$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("^DallasBaptistUniv$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("^巴拉圭高等戰略研究院$", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu1 <-
          if_else(
            grepl("DomusAcademy", flag_person$mdegreeu1),
            0,
            flag_person$err_mdegreeu1
          )
        flag_person$err_mdegreeu1 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("Universidad$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("Conservatory$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("TheNewSchool$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("Universitat$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("StellenboschUni$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("^DallasBaptistUniv$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("^巴拉圭高等戰略研究院$", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_mdegreeu2 <-
          if_else(
            grepl("DomusAcademy", flag_person$mdegreeu2),
            0,
            flag_person$err_mdegreeu2
          )
        flag_person$err_mdegreeu2 <-
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("Conservatory", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("TheNewSchool", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("StellenboschUni$", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("^IstitutoSecoli$", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl(
              "^CityandGuildsofLondonArtSchool$",
              flag_person$bdegreeu1
            ),
            0,
            flag_person$err_bdegreeu1
        ...
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("^日本國立埼玉大學教養學部$", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
          if_else(
            grepl("^日本國立奈良教育大學（學院）$", flag_person$bdegreeu1),
            0,
            flag_person$err_bdegreeu1
          )
        flag_person$err_bdegreeu1 <-
        ...
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("Conservatory", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
        ...
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("TheNewSchool", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
        ...
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("StellenboschUni$", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
        ...
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("^IstitutoSecoli$", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
        ...
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("^日本國立埼玉大學教養學部$", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
          if_else(
            grepl("^日本國立奈良教育大學（學院）$", flag_person$bdegreeu2),
            0,
            flag_person$err_bdegreeu2
          )
        flag_person$err_bdegreeu2 <-
        ...
        flag_person$err_adegreeu1 <-
          if_else(
            grepl("Conservatory", flag_person$adegreeu1),
            0,
            flag_person$err_adegreeu1
          )
        flag_person$err_adegreeu1 <-
        ...
        flag_person$err_adegreeu1 <-
          if_else(
            grepl("TheNewSchool", flag_person$adegreeu1),
            0,
            flag_person$err_adegreeu1
          )
        flag_person$err_adegreeu1 <-
        ...
        flag_person$err_adegreeu1 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$adegreeu1),
            0,
            flag_person$err_adegreeu1
          )
        flag_person$err_adegreeu1 <-
        ...
        flag_person$err_adegreeu1 <-
          if_else(
            grepl("^仁德高級醫事職業學校$", flag_person$adegreeu1),
            0,
            flag_person$err_adegreeu1
          )
        #"慈惠醫護管理專科學校"的前身
        flag_person$err_adegreeu1 <-
          if_else(
            grepl("^私立慈惠謢理助產學校$", flag_person$adegreeu1),
            0,
            flag_person$err_adegreeu1
          )
        flag_person$err_adegreeu1 <-
        ...
        flag_person$err_adegreeu2 <-
          if_else(
            grepl("Conservatory", flag_person$adegreeu2),
            0,
            flag_person$err_adegreeu2
          )
        flag_person$err_adegreeu2 <-
        ...
        flag_person$err_adegreeu2 <-
          if_else(
            grepl("TheNewSchool", flag_person$adegreeu2),
            0,
            flag_person$err_adegreeu2
          )
        flag_person$err_adegreeu2 <-
        ...
        flag_person$err_adegreeu2 <-
          if_else(
            grepl("^TUDarmstadt$", flag_person$adegreeu2),
            0,
            flag_person$err_adegreeu2
          )
        flag_person$err_adegreeu2 <-
        ...
        flag_person$err_adegreeu2 <-
          if_else(
            grepl("^仁德高級醫事職業學校$", flag_person$adegreeu2),
            0,
            flag_person$err_adegreeu2
          )
        #"慈惠醫護管理專科學校"的前身
        flag_person$err_adegreeu2 <-
          if_else(
            grepl("^私立慈惠謢理助產學校$", flag_person$adegreeu2),
            0,
            flag_person$err_adegreeu2
          )
        flag_person$err_adegreeu2 <-
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          flag83 <- flag83 %>%
            ...
            mutate(
              flag83 = paste(
                flag83,
                "（請確認上述人員是否退休、退伍或因故離職，若是，則不需填列至本次教員資料表或職員（工）資料表，並請務必依欄位說明確認離退職類別）",
                sep = ""
              )
            )
        } else{
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          flag93 <- flag93 %>%
            ...
            mutate(flag93 = if_else(
              ...
              paste0(
                flag93,
                "（查貴校上一學年所填資料，上述人員未在貴校教職員(工)資料中，請確認上述人員是否於112年2月1日-112年7月31日有退休或因故離職之情形，或是否屬於貴校教職員(工)，併請確認貴校教職員工名單是否完整正確。）"
              ),
              #私立
              paste0(
                flag93,
                "（查貴校上一學年所填資料，上述人員未在貴校教職員(工)資料中，請確認上述人員是否於111年10月1日-112年7月31日有退休或因故離職之情形，或是否屬於貴校教職員(工)，併請確認貴校教職員工名單是否完整正確。）"
              )
            )) #公立
        } else{
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          flag86 <- flag86 %>%
            ...
            mutate(flag86 = if_else(
              ...
              paste0(
                flag86,
                "（經比對貴校上一學年所填資料，上述人員並未出現於本學期的教員資料表或職員(工)資料表，請確認渠等是否於111學年度第二學期（112年2月1日-112年7月31日）退休或因故離職等，若於該期間退休或因故離職等，應於離退教職員(工)資料表填寫資料。如非於該期間退休或因故離職，或已介聘、調至他校，請來電告知。）"
              ),
              #私立
              paste0(
                flag86,
                "（經比對貴校上一學年所填資料，上述人員並未出現於本學期的教員資料表或職員(工)資料表，請確認渠等是否於111年10月1日-112年7月31日退休或因故離職等，若於該期間退休或因故離職等，應於離退教職員(工)資料表填寫資料。如非於該期間退休或因故離職，或已介聘、調至他校，請來電告知。）"
              )
            )) #公立
        ...
        flag_person_wide_flag95$err_staff <-
          (
            flag_person_wide_flag95$count_staff - flag_person_wide_flag95$count_staff_1
          ) / flag_person_wide_flag95$count_staff
        ...
        if (dim(flag_person %>% subset(err_flag == 1))[1] != 0) {
          ...
          flag100 <- flag100 %>%
            ...
            mutate(
              flag100 = paste(
                flag100,
                "（校長『本校到職前學校服務總年資』似偏少，請確認校長就任之『本校到職日期』，以及過去在本校及他校擔任教師與主任等全職工作之『本校到職前學校服務總年資』。）",
                sep = ""
              )
            )
        } else{
        ...
        openxlsx::write.xlsx(
          check02,
          file = "./dist/edhr-112t1-check_print-人事.xlsx",
          rowNames = FALSE,
          overwrite = TRUE
        )
